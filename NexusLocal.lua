-- NEXUS:RT | LOCAL SCRIPTS | FULLY FIXED | SIMPLE EXECUTOR FRIENDLY
...
getgenv().NexusSettings = {
    Width = 220, Height = 280, Color = "GREEN",
    Colors = {
...
-- TOGGLE MED ÖGON-ICON
local tog = Instance.new("TextButton")
tog.Size=UDim2.new(0,44,0,44) tog.Position=UDim2.new(0.90,0,0.05,0) tog.BackgroundColor3=BG 
...
eyeIcon.Size = UDim2.new(0, 26, 0, 26)
eyeIcon.Position = UDim2.new(0.5, -13, 0.5, -13)
...
main.Size=UDim2.new(0,0,0,0) main.Position=UDim2.new(0.05,0,0.12,0) main.AnchorPoint=Vector2.new(0,0) main.BackgroundColor3=BG main.BackgroundTransparency=1 main.Active=true main.Draggable=true main.Visible=false main.ClipsDescendants=true main.Parent=sg
...
-- HEADER MED UNDERRUBRIK
local hdr=Instance.new("Frame") hdr.Size=UDim2.new(1,-12,0,38) hdr.Position=UDim2.new(0,6,0,6) hdr.BackgroundTransparency=1 hdr.Parent=main
local ttl=Instance.new("TextLabel") ttl.Size=UDim2.new(0.6,0,0,22) ttl.Position=UDim2.new(0,0,0,0) ttl.BackgroundTransparency=1 ttl.Text="NEXUS:RT" ttl.Font=FONT ttl.TextScaled=true ttl.TextXAlignment=Enum.TextXAlignment.Left ttl.Parent=hdr
local subTtl=Instance.new("TextLabel") subTtl.Size=UDim2.new(0.6,0,0,14) subTtl.Position=UDim2.new(0,0,0,22) subTtl.BackgroundTransparency=1 subTtl.Text="LOCAL SCRIPTS" subTtl.Font=FONT subTtl.TextScaled=true subTtl.TextColor3=TXTDIM subTtl.TextXAlignment=Enum.TextXAlignment.Left subTtl.Parent=hdr
...
local hl=Instance.new("Frame") hl.Size=UDim2.new(1,-12,0,1) hl.Position=UDim2.new(0,6,0,48) hl.BackgroundColor3=SET.Colors[SET.Color].Dim hl.BackgroundTransparency=0.6 hl.Parent=main

-- TIMER
local timerLabel=Instance.new("TextLabel") timerLabel.Size=UDim2.new(1,-12,0,18) timerLabel.Position=UDim2.new(0,6,0,54) timerLabel.BackgroundTransparency=1 timerLabel.Text="" timerLabel.TextColor3=SET.Colors[SET.Color].Main timerLabel.Font=FONT timerLabel.TextScaled=true timerLabel.TextXAlignment=Enum.TextXAlignment.Center timerLabel.Parent=main

-- TABS
local tabs={}
local function makeTab(n,x,a)
    local t=Instance.new("TextButton") t.Size=UDim2.new(0.3,0,0,24) t.Position=UDim2.new(x,0,0,76) t.BackgroundColor3=a and BG or BGDARK t.BackgroundTransparency=a and 0 or 0.4 t.Text="["..n:upper().."]" t.TextColor3=a and SET.Colors[SET.Color].Main or TXTDIM t.Font=FONT t.TextScaled=true t.Parent=main
...
local t1=makeTab("MAIN",0.03,true) local t2=makeTab("LIST",0.355,false) local t3=makeTab("SET",0.68,false)

-- ✅ FIX: CONTENT ÄR NU RÄTT STORLEK OCH POSITION (KNAPPAR SYNS!)
local content=Instance.new("Frame") content.Size=UDim2.new(1,-12,0,160) content.Position=UDim2.new(0,6,0,104) content.BackgroundTransparency=1 content.ClipsDescendants=false content.Parent=main
local p1=Instance.new("Frame") p1.Size=UDim2.new(1,0,1,0) p1.BackgroundTransparency=1 p1.Visible=true p1.Parent=content

-- SCROLL FRAME FÖR LIST
...
local sb=Instance.new("TextBox") sb.Size=UDim2.new(1,0,0,24) sb.Text="[SEARCH]" sb.TextColor3=TXT sb.Font=FONT sb.TextScaled=true sb.ClearTextOnFocus=true sb.Parent=sf
Instance.new("UICorner",sb).CornerRadius=UDim.new(0,3) Instance.new("UIStroke",sb).Thickness=1

local p3=Instance.new("ScrollingFrame") p3.Size=UDim2.new(1,0,1,0) p3.BackgroundTransparency=1 p3.Visible=false p3.ClipsDescendants=true p3.ScrollBarThickness=2 p3.CanvasSize=UDim2.new(0,0,0,280) p3.Parent=content
local p3Inner=Instance.new("Frame") p3Inner.Size=UDim2.new(1,0,0,280) p3Inner.BackgroundTransparency=1 p3Inner.Parent=p3

-- STATUS
local bot=Instance.new("TextLabel") bot.Size=UDim2.new(1,-12,0,16) bot.Position=UDim2.new(0,6,1,-22) bot.BackgroundTransparency=1 bot.Text="STATUS: READY" bot.TextColor3=SET.Colors[SET.Color].Main bot.Font=FONT bot.TextScaled=true bot.TextXAlignment=Enum.TextXAlignment.Left bot.Parent=main
...
-- ✅ FIX: KEY UI ÄR NU STOR (INGEN LILLA SKIT LÄNGRE!)
...
local ks=Instance.new("UIStroke",KeyUI) ks.Thickness=1.4 ks.Color=SET.Colors[SET.Color].Dim ks.Transparency=0.4

local kT=Instance.new("TextLabel",KeyUI) kT.Size=UDim2.new(1,-30,0,40) kT.Position=UDim2.new(0,15,0,10) kT.BackgroundTransparency=1 kT.Text="NEXUS:RT" kT.TextColor3=SET.Colors[SET.Color].Main kT.Font=FONT kT.TextScaled=true
local kSub=Instance.new("TextLabel",KeyUI) kSub.Size=UDim2.new(1,-30,0,20) kSub.Position=UDim2.new(0,15,0,48) kSub.BackgroundTransparency=1 kSub.Text="LOCAL SCRIPTS" kSub.TextColor3=TXTDIM kSub.Font=FONT kSub.TextScaled=true
local kD=Instance.new("TextLabel",KeyUI) kD.Size=UDim2.new(1,-30,0,22) kD.Position=UDim2.new(0,15,0,74) kD.BackgroundTransparency=1 kD.Text="ENTER ACCESS KEY" kD.TextColor3=TXTDIM kD.Font=FONT kD.TextScaled=true

local kI=Instance.new("TextBox") kI.Size=UDim2.new(1,-30,0,48) kI.Position=UDim2.new(0,15,0,102) kI.BackgroundColor3=BGDARK kI.Text="" kI.PlaceholderText="Type key here..." kI.TextColor3=TXT kI.Font=FONT kI.TextScaled=true kI.ClearTextOnFocus=false kI.Parent=KeyUI
Instance.new("UICorner",kI).CornerRadius=UDim.new(0,6)
local kis=Instance.new("UIStroke",kI) kis.Thickness=1.2 kis.Color=SET.Colors[SET.Color].Dim

local kS=Instance.new("TextLabel",KeyUI) kS.Size=UDim2.new(1,-30,0,20) kS.Position=UDim2.new(0,15,0,156) kS.BackgroundTransparency=1 kS.Text="" kS.TextColor3=TXTDIM kS.Font=FONT kS.TextScaled=true

-- COPY KNAPP (STOR OCH LÄTT ATT TRYCKA PÅ MOBIL)
local copyBtn = Instance.new("TextButton")
copyBtn.Name = "CopyEzKey"
copyBtn.Size = UDim2.new(1, -30, 0, 40)
copyBtn.Position = UDim2.new(0, 15, 0, 182)
copyBtn.BackgroundColor3 = BGDARK
copyBtn.BackgroundTransparency = 0.2
copyBtn.Text = "[ COPY EZ KEY ]"
copyBtn.TextColor3 = SET.Colors[SET.Color].Main
copyBtn.Font = FONT
...
copyStroke.Color = SET.Colors[SET.Color].Dim
copyStroke.Transparency = 0.5
copyStroke.Parent = copyBtn

copyBtn.MouseEnter:Connect(function() TweenProp(copyBtn, "BackgroundTransparency", 0) TweenProp(copyStroke, "Transparency", 0) end)
copyBtn.MouseLeave:Connect(function() TweenProp(copyBtn, "BackgroundTransparency", 0.2) TweenProp(copyStroke, "Transparency", 0.5) end)
copyBtn.MouseButton1Click:Connect(function()
    local ok = pcall(function() setclipboard(EZ_KEY_LINK) end)
    if ok then
        Notify("Link copied!", SET.Colors[SET.Color].Main)
        copyBtn.Text = "[ COPIED! ]"
        task.delay(1.5, function() if copyBtn.Parent then copyBtn.Text = "[ COPY EZ KEY ]" end end)
    else Notify("Copy failed", ERR_COL) end
end)
...
tog.MouseButton1Click:Connect(function()
    uiOpen=not uiOpen
    if uiOpen then 
        eyeIcon.Image = ICON_OPEN eyeIcon.ImageColor3 = SET.Colors[SET.Color].Main
        main.Visible=true TweenProp(main,"Size",UDim2.new(0,SET.Width,0,SET.Height),TWEEN_SIZE) TweenProp(main,"BackgroundTransparency",0,TWEEN_FADE)
    else 
        eyeIcon.Image = ICON_CLOSED eyeIcon.ImageColor3 = TXTDIM
        TweenProp(main,"Size",UDim2.new(0,0,0,0),TWEEN_SIZE) TweenProp(main,"BackgroundTransparency",1,TWEEN_FADE) 
...
function verifyKey()
    ...
    if v==CORRECT_KEY:lower() then
        ...
        Notify("Access granted", OK_COL) saveAccess() task.wait(1) openMainUI(ACCESS_DURATION)
    else
...
local function ApplySettings(tw)
    ...
    content.Size=UDim2.new(1,-12,0,SET.Height-140)
    ...
    if copyBtn and copyBtn.Parent then copyBtn.TextColor3=C copyStroke.Color=D ks.Color=D kis.Color=D end
...
local function switchTab(a)
    ...
    if a=="MAIN" then p1.Visible=true elseif a=="LIST" then sf.Visible=true elseif a=="SET" then p3.Visible=true end
end
t1.MouseButton1Click:Connect(function() switchTab("MAIN") end)
t2.MouseButton1Click:Connect(function() switchTab("LIST") end)
t3.MouseButton1Click:Connect(function() switchTab("SET") end)

-- ==============================================
-- ✅ ENKEL LOCAL SCRIPTS SYSTEM (FUNKAR I ALLA EXECUTORS!)
-- ==============================================
local allScripts = {}
local selScript = nil
local scriptBtns = {}
local isScanning = false
local execDelay = 0.08

-- SÖK I LISTAN
sb.Changed:Connect(function(p)
    if p~="Text" then return end local q=sb.Text:lower()
    if q=="[search]" or q=="" then for b in pairs(scriptBtns) do b.Visible=true end return end
    for b,s in pairs(scriptBtns) do b.Visible=s.Name:lower():find(q,1,true)~=nil end
end)

-- KÖR ETT LOCAL SCRIPT (ENKEL OCH UNIVERSELL METOD)
local function runScript(scr)
    if not scr or not scr:IsA("LocalScript") then return false end
    local ok, err = pcall(function()
        -- loadstring fungerar i ALLA executors, inget krångel!
...
-- 1. SCAN: HITTA ALLA LOCAL SCRIPTS
local function scanScripts()
    if isScanning then Notify("Already scanning", BTN_COLORS.SCAN) return end
    isScanning=true getgenv().TestRunning=false
    Notify("Scanning scripts...", BTN_COLORS.SCAN)
    for b in pairs(scriptBtns) do pcall(function() b:Destroy() end) end scriptBtns={} allScripts={} selScript=nil
    bot.Text="🔍 SCANNING..." sf.CanvasSize=UDim2.new(0,0,0,30)

    -- Söker i alla vanliga platser
    local char = plr.Character or plr.CharacterAdded:Wait()
    local locs = {pg, plr.PlayerScripts, plr.Backpack, char, rs, game.ReplicatedFirst, game.StarterGui, workspace}
    for _,l in pairs(locs) do
        pcall(function()
            for _,o in pairs(l:GetDescendants()) do
                if o:IsA("LocalScript") and not table.find(allScripts,o) then table.insert(allScripts,o) end
            end
        end)
    end
    ...
    for i,s in ipairs(allScripts) do
        local b=Instance.new("TextButton") b.Size=UDim2.new(1,-4,0,26) b.Position=UDim2.new(0,2,0,i*30+28) b.BackgroundColor3=BGDARK b.BackgroundTransparency=0.3 b.Text=" "..s.Name b.TextColor3=TXT b.Font=FONT b.TextScaled=true b.TextXAlignment=Enum.TextXAlignment.Left b.Parent=sf
        ...
        b.MouseButton1Click:Connect(function()
            for ob in pairs(scriptBtns) do ob.BackgroundColor3=BGDARK ob.BackgroundTransparency=0.3 ob.TextColor3=TXT ob.UIStroke.Color=SET.Colors[SET.Color].Dim ob.UIStroke.Transparency=0.7 end
            selScript=s b.BackgroundColor3=Color3.fromRGB(0,40,30) b.BackgroundTransparency=0.1 b.TextColor3=SET.Colors[SET.Color].Main bs.Color=SET.Colors[SET.Color].Main bs.Transparency=0
            bot.Text="🎯 SCRIPT: "..s.Name:sub(1,14)
            Notify("Selected: "..s.Name:sub(1,12), BTN_COLORS.TARGET)
        end)
    end
    sf.CanvasSize=UDim2.new(0,0,0,math.max(#allScripts*30+60,100))
    bot.Text="✅ FOUND: "..#allScripts.." | READY" isScanning=false
    Notify("Found "..#allScripts.." scripts", BTN_COLORS.SCAN)
end

-- 2. TARGET: KÖR VALD
local function execSelected()
    if not selScript then
        bot.Text="❌ NO SCRIPT"
        Notify("Select a script first", BTN_COLORS.TARGET)
        return
    end
    local ok, err = runScript(selScript)
    if ok then
        bot.Text="✅ RAN: "..selScript.Name:sub(1,12)
        Notify("Executed: "..selScript.Name:sub(1,12), BTN_COLORS.TARGET)
    else
        bot.Text="❌ FAILED"
        Notify("Script error", ERR_COL)
    end
end

-- 3. MASS: KÖR ALLA
local function execAll()
    if #allScripts == 0 then scanScripts() return end
    getgenv().TestRunning = true
    bot.Text="💥 RUNNING ALL ("..#allScripts..")"
    Notify("Mass exec: "..#allScripts, BTN_COLORS.MASS)
    local okCount = 0
    task.spawn(function()
        for _, s in ipairs(allScripts) do
            if not getgenv().TestRunning then return end
            if runScript(s) then okCount += 1 end
            bot.Text="💥 "..okCount.."/"..#allScripts
            task.wait(execDelay)
        end
        if getgenv().TestRunning then
            getgenv().TestRunning = false
            bot.Text="✅ DONE: "..okCount
            Notify("Done: "..okCount.."/"..#allScripts, OK_COL)
        end
    end)
end

-- 4. STOP
local function stopExec()
    getgenv().TestRunning = false
    bot.Text="⏹️ STOPPED"
    Notify("Stopped", BTN_COLORS.STOP)
end

-- ✅ FIX: KNAPPARNA ÄR NU I P1 OCH SYNS I MAIN!
local function makeBtn(p,txt,c,y,fn)
    local b=Instance.new("TextButton") b.Size=UDim2.new(0.46,0,0,36) b.Position=UDim2.new(y%2==0 and 0 or 0.54,0,0,math.floor(y/2)*44) b.BackgroundColor3=BGDARK b.BackgroundTransparency=0.25 b.Text="["..txt.."]" b.TextColor3=c b.Font=FONT b.TextScaled=true b.Parent=p
...
makeBtn(p1,"SCAN",BTN_COLORS.SCAN,0,scanScripts)
makeBtn(p1,"TARGET",BTN_COLORS.TARGET,1,execSelected)
makeBtn(p1,"MASS",BTN_COLORS.MASS,2,execAll)
makeBtn(p1,"STOP",BTN_COLORS.STOP,3,stopExec)

-- TEMAKNAPPAR I SET
for i,n in ipairs({"GREEN","RED","BLUE","YELLOW"}) do
    local c=SET.Colors[n].Main local b=Instance.new("TextButton") b.Size=UDim2.new(0.22,0,0,30) b.Position=UDim2.new((i-1)*0.26,0,0,28) b.BackgroundColor3=c b.Text=n b.TextColor3=Color3.new(0,0,0) b.Font=FONT b.TextScaled=true b.Parent=p3Inner
    Instance.new("UICorner",b).CornerRadius=UDim.new(0,4)
    b.MouseButton1Click:Connect(function() 
...
local function makeSlider(p,mn,mx,key,y)
    local l=Instance.new("TextLabel") l.Size=UDim2.new(1,0,0,18) l.Position=UDim2.new(0,0,0,y) l.BackgroundTransparency=1 l.Text=key:upper()..": "..mn.."-"..mx.." | "..SET[key] l.TextColor3=TXTDIM l.Font=FONT l.TextScaled=true l.Parent=p
    local s=Instance.new("TextButton") s.Size=UDim2.new(1,0,0,16) s.Position=UDim2.new(0,0,0,y+22) s.BackgroundColor3=BGDARK s.AutoButtonColor=false s.Text="" s.Parent=p Instance.new("UICorner",s).CornerRadius=UDim.new(0,8)
    local f=Instance.new("Frame") f.Size=UDim2.new((SET[key]-mn)/(mx-mn),0,1,0) f.BackgroundColor3=SET.Colors[SET.Color].Main f.Parent=s Instance.new("UICorner",f).CornerRadius=UDim.new(0,8)
...
makeSlider(p3Inner,200,350,"Width",80)
makeSlider(p3Inner,260,400,"Height",130)

-- WIPE KNAPP
local wipe=Instance.new("TextButton") wipe.Size=UDim2.new(1,0,0,36) wipe.Position=UDim2.new(0,0,0,220) wipe.BackgroundColor3=BTN_COLORS.MASS wipe.Text="WIPE UI" wipe.TextColor3=Color3.new(1,1,1) wipe.Font=FONT wipe.TextScaled=true wipe.Parent=p3Inner
Instance.new("UICorner",wipe).CornerRadius=UDim.new(0,4)
...
-- START (KEY UI ÄR NU 360x240 = STOR!)
local hasAccess,timeLeft=checkAccess()
if hasAccess then openMainUI(timeLeft)
else KeyUI.Visible=true TweenProp(KeyUI,"Size",UDim2.new(0,360,0,240),TWEEN_SIZE) TweenProp(KeyUI,"BackgroundTransparency",0,TWEEN_FADE) end
ApplySettings(false)
print("✅ NEXUS:RT LOCAL SCRIPTS - ALL FIXED")
